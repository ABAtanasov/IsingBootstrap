...
# for obtaining the bounds on OPE coefficients
# -----------------------------------------------------------------
...
def make_SDP_cmin(deltas, theta=None):
    delta_s = context(deltas[0])
    delta_e = context(deltas[1])
    oddgap  = context(sigpgap)
    evengap = context(epspgap)
    pvms=[]
    gaps={("even",0):evengap,("odd+",0):oddgap}
    for spin in range(0,lmax):
        if not spin%2:
            pvms.append(make_F(deltas,"even",spin,gaps))
            pvms.append(make_F(deltas,"odd+",spin,gaps))
    ...
    obj=[]
    for m,x in zip(epsilon_contribution,sigma_contribution):
        sn = context(np.sin(theta))
        cs = context(np.cos(theta))
        #ans = (m[0][0]+x) * (cs ** 2) + m[0][1] * sn * cs + m[1][0] * sn * cs + m[1][1] * (sn ** 2)
        #norm.append(ans)
        norm.append(((m[0][0]+x)*cs**2 + m[0][1]*sn*cs + m[1][0]*sn*cs + m[1][1]*sn**2)) 
    for v in make_F(deltas,"even",0,{},Delta=0):
...
def check(deltas, checkpoint, theta=None, f=None):
    ...
    sol = re.compile(r'primalObjective *= *([^ ]+) *$', re.MULTILINE) \
        .search(out).groups()[0]
    f.write("OPE Bound for ({},{}, {}): {}\n".format(deltas[0], deltas[1], theta, float(sol)))
